 ...
 * Runs WAV/AIFF → MP3 conversion off the main thread to keep the UI responsive.
 *
 * Protocol:
 *   main → worker : { entries: WorkerEntry[] }
 *   worker → main : { ok: true,  entries: WorkerEntry[] }
 *   worker → main : { ok: false, message: string }
 *
 * lame.min.js is loaded via fetch (no importScripts in module workers).
 */
...
// ─── Audio parsers (mirrored from compress-wav.ts) ───────────────────────────
...
function toInt16Samples(buffer: ArrayBuffer, info: AudioInfo): Int16Array {
  ...
  if (bitsPerSample === 16) {
    ...
    for (let i = 0; i < count; i++) out[i] = view.getInt16(dataOffset + i * 2, !bigEndian);
    return out;
  }
  ...
  if (bitsPerSample === 24) {
    ...
    for (let i = 0; i < count; i++) {
      const b   = [view.getUint8(dataOffset + i*3), view.getUint8(dataOffset + i*3+1), view.getUint8(dataOffset + i*3+2)];
      const val = bigEndian ? (b[0] << 16) | (b[1] << 8) | b[2] : b[0] | (b[1] << 8) | (b[2] << 16);
  ...
  if (bitsPerSample === 32) {
    ...
    const out   = new Int16Array(count);
    for (let i = 0; i < count; i++) out[i] = view.getInt32(dataOffset + i * 4, !bigEndian) >> 16;
    return out;
  }
...
let lameModule: LameModule | null = null;
...
async function getLamejs(): Promise<LameModule> {
  ...
  new Function('window', code + '\nwindow.lamejs = lamejs;')(windowShim);
...
async function encodeToMp3(buffer: ArrayBuffer, fileName: string, lame: LameModule, kbps = 192): Promise<{ buffer: ArrayBuffer; name: string }> {
  const view = new DataView(buffer);
  const tag  = String.fromCharCode(view.getUint8(0), view.getUint8(1), view.getUint8(2), view.getUint8(3));
  const info = tag === 'FORM' ? parseAiff(buffer) : parseWav(buffer);
  const { channels, sampleRate } = info;

  if (channels !== 1 && channels !== 2) throw new Error(`지원하지 않는 채널 수: ${channels}ch`);
  ...
  const mp3Buffer = await new Blob(chunks, { type: 'audio/mpeg' }).arrayBuffer();
  const mp3Name   = fileName.replace(/\.(wav|aif|aiff|aifc)$/i, '.mp3');
  return { buffer: mp3Buffer, name: mp3Name };
}
...
self.onmessage = async (e: MessageEvent<{ entries: WorkerEntry[] }>) => {
  try {
    ...
    for (const entry of entries) {
      ...
      if (COMPRESS_EXTS.has(ext)) {
        const { buffer, name } = await encodeToMp3(entry.buffer, entry.name, lame);
        result.push({ buffer, name, label: entry.label, fileType: 'audio' });
      } else {
        result.push(entry);
      }
    }

    const transferList = result.map(r => r.buffer);
    self.postMessage({ ok: true, entries: result }, { transfer: transferList });
  } catch (err) {
