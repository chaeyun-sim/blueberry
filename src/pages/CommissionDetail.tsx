...
const CommissionDetailContent = () => {
	...
	const handleTemplateDownload = async () => {
		...
		try {
			...
			const workTitle = linked ? (linked.title ?? '') : (commission.title ?? '');
			...
			const meta = await analyzeMusicMeta(
				workTitle,
				composer,
				movementTitle || undefined,
			);
			...
			const canonicalForSub =
				strippedCanonical && strippedCanonical !== collectionRef
					? strippedCanonical
					: '';
			...
			const mxlSubtitle =
				rawSubtitle && rawSubtitle !== mxlTitle
					? rawSubtitle
					: hasMovementContext
						? 'Untitled'
						: undefined;
	...
	return (
		...
		>
			...
			<Card className='border-border/50 mb-10 md:mb-0'>
				<CardContent className='p-5 space-y-3'>
					<button
						type='button'
