...
export function CalendarDayCell({
	...
	return (
		<div
			...
			onClick={() => { if (currentMonth) onSelect(dateStr); }}
			...
			<div className='flex items-center justify-between mb-1 shrink-0'>
				<span
					className={cn(
						...
						currentMonth && isSat && 'text-[hsl(var(--warning))]',
			...
			<div className='flex flex-col gap-0.5 overflow-hidden flex-1 min-h-0'>
				{dayCommissions.slice(0, 3).map((c) => (
					<button
						...
						key={c.id}
						draggable
						onDragStart={(e) => onDragStart(e as unknown as DragEvent<HTMLButtonElement>, c.id)}
						onDragEnd={onDragEnd}
						className={cn(
