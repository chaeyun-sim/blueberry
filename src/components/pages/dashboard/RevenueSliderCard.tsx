...
function RevenueSliderCard() {
	...
	const revenueSlides = [
		{
			label: '전체 누적 매출',
			value: getNetAmount(summary?.totalRevenue ?? 0),
			sub: null,
			up: true,
		},
		{
			label: '지난 달 매출',
			value: getNetAmount(summary?.lastMonthRevenue ?? 0),
			sub:
				summary?.revenueVsLastMonth != null
	...
	useEffect(() => {
		const timer = setInterval(
			() => setSlideIdx((p) => (p + 1) % revenueSlides.length),
			4000,
		);
			...
			<AnimatePresence mode='wait'>
				...
				>
					...
					{revenueSlides[slideIdx].sub ? (
						<p
							className={`text-xs flex items-center gap-0.5 ${revenueSlides[slideIdx].up ? 'text-[hsl(var(--success))]' : 'text-destructive'}`}
						>
