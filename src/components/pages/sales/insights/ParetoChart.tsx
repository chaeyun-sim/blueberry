...
function ParetoChart() {
  ...
  const point80 = useMemo(() => items.find(d => d.cumulativeShare >= 80), [items]);
  const point20 = useMemo(() => items.find(d => d.songShare >= 20), [items]);
  ...
  return (
    <Card className='border-border/50'>
      <CardHeader className='pb-2'>
        ...
        <p className='text-xs text-muted-foreground'>상위 몇 %의 곡이 전체 매출을 책임지는지</p>
      </CardHeader>
      <CardContent>
        {items.length === 0 ? (
                    ...
                    return (
                      <div className='rounded-lg border border-border bg-background px-3 py-2 text-xs shadow-md space-y-0.5'>
                        ...
                        <p className='text-muted-foreground'>
                          매출 {formatCurrency(getNetAmount(d.revenue))}
                        </p>
                        <p className='text-muted-foreground'>
