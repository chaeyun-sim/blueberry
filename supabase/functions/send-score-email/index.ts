...
Deno.serve(async (req) => {
	...
	try {
		...
		if (commErr || !commission) {
			return new Response(JSON.stringify({ error: '의뢰를 찾을 수 없어요' }), {
				status: 404,
				headers: corsHeaders,
			});
		}
		...
		if (arrErr || !arrangements || arrangements.length === 0) {
			return new Response(JSON.stringify({ error: '연결된 악보가 없어요' }), {
				status: 404,
				headers: corsHeaders,
			});
		}
		...
		if (filesErr || !files || files.length === 0) {
			return new Response(JSON.stringify({ error: '첨부할 파일이 없어요' }), {
				status: 404,
				headers: corsHeaders,
			});
		}
